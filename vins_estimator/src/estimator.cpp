...
Estimator::Estimator() : f_manager{Rs} {
  ...
  std::string model_path =
      "/home/hkr/mnt/Code/0906/libtorch_demo/model/rnin.pt";
  imu_nn_model_ = torch::jit::load(model_path);
  imu_nn_model_.to(at::kCUDA);
}
...
void Estimator::processIMUNeuralNetwork(
    const std::vector<sensor_msgs::Imu> &imu_buf, double current_img_time) {
  float imu_data_for_network[6000];
  int idx = 0;
  for (int i = 0; i < 10; i++) {
    int wx_begin_idx = i * 600;
    int wy_begin_idx = i * 600 + 100;
    int wz_begin_idx = i * 600 + 200;
    int ax_begin_idx = i * 600 + 300;
    int ay_begin_idx = i * 600 + 400;
    int az_begin_idx = i * 600 + 500;
    for (int j = 0; j < 100; j++) {
      imu_data_for_network[wx_begin_idx++] = imu_buf[idx].angular_velocity.x;
      imu_data_for_network[wy_begin_idx++] = imu_buf[idx].angular_velocity.y;
      imu_data_for_network[wz_begin_idx++] = imu_buf[idx].angular_velocity.z;
      imu_data_for_network[ax_begin_idx++] = imu_buf[idx].linear_acceleration.x;
      imu_data_for_network[ay_begin_idx++] = imu_buf[idx].linear_acceleration.y;
      imu_data_for_network[az_begin_idx++] = imu_buf[idx].linear_acceleration.z;
      idx++;
    }
  }
  auto imu_tensor =
      torch::from_blob(imu_data_for_network, {1, 10, 6, 100}, torch::kFloat);
  std::vector<torch::jit::IValue> inputs;
  torch::DeviceType device_type = at::kCUDA;
  inputs.emplace_back(imu_tensor.to(device_type));
  auto outputs = imu_nn_model_.forward(inputs).toTuple();
  ...
  // if (last_imu_nn_output[0] < -1000) {
  //   last_imu_nn_output[0] = v[0];
  //   last_imu_nn_output[1] = v[1];
  //   last_imu_nn_output[2] = v[2];
  //   return;
  // }

  if (!imu_network_measurements[frame_count]) {
    imu_network_measurements[frame_count] = new ImuNeuralNetworkBase();
  }
  double delta_time = current_img_time - Headers[frame_count - 1].stamp.toSec();
  float vx = out1[0][9][0].item().toFloat();
  float vy = out1[0][9][1].item().toFloat();
  float vz = out1[0][9][2].item().toFloat();
  float covx = out2[0][9][0].item().toFloat();
  float covy = out2[0][9][1].item().toFloat();
  float covz = out2[0][9][2].item().toFloat();
  imu_network_measurements[frame_count]->setTranslationAndCov(
      Eigen::Vector3d{vx * delta_time,
                      vy * delta_time,
                      vz * delta_time},
      Eigen::Vector3d{covx, covy, covz});

  std::string imu_output_path = "/home/hkr/mnt/VINS-ws/out_put/imu_nn.csv";
  ofstream foutC(imu_output_path, ios::app);
  foutC.setf(ios::fixed, ios::floatfield);
  foutC.precision(9);
  foutC << Headers[frame_count - 1].stamp.toSec() << " " << current_img_time
        << " ";
  foutC.precision(5);
  foutC << vx * delta_time << " " <<  vy * delta_time << " "
        << vz * delta_time << "  " << std::endl;
  foutC.close();
  return;
}
...
void Estimator::optimization() {
  ...
  for (int i = 0; i < WINDOW_SIZE; i++) {
    ...
    if (imu_network_measurements[j] != nullptr) {
      IMUNeuralNetworkFactor *imu_neural_network_factor =
          new IMUNeuralNetworkFactor(imu_network_measurements[j]);
      problem.AddResidualBlock(imu_neural_network_factor, NULL, para_Pose[i],
  ...
  if (marginalization_flag == MARGIN_OLD) {
...

    {
      ...
      ////////////////////////////////////////////////////////////////////////////
      if (imu_network_measurements[1] != nullptr) {
        IMUNeuralNetworkFactor *imu_neural_network_factor =
            new IMUNeuralNetworkFactor(imu_network_measurements[1]);
        ResidualBlockInfo *imu_nn_residual_block_info = new ResidualBlockInfo(
            imu_neural_network_factor, NULL,
            vector<double *>{para_Pose[0], para_SpeedBias[0], para_Pose[1],
                             para_SpeedBias[1]},
            vector<int>{0, 1});
        marginalization_info->addResidualBlockInfo(imu_nn_residual_block_info);
      }
      ///////////////////////////////////////////////////////////////////////////
    }
  ...
  if (marginalization_flag == MARGIN_OLD) {
    ...
    if (frame_count == WINDOW_SIZE) {
      for (int i = 0; i < WINDOW_SIZE; i++) {
        ...
        //////////////////////////////////////////////////////
        std::swap(imu_network_measurements[i], imu_network_measurements[i + 1]);
        /////////////////////////////////////
      ...
      ////////////////////////////
      if (imu_network_measurements[WINDOW_SIZE] != nullptr) {
        delete imu_network_measurements[WINDOW_SIZE];
        imu_network_measurements[WINDOW_SIZE] = new ImuNeuralNetworkBase();
      }
    ...
    if (frame_count == WINDOW_SIZE) {
      ...
      ///////////////////////////////////////////////////////////////////////////////////////
      if (imu_network_measurements[frame_count - 1] != nullptr &&
          imu_network_measurements[frame_count] != nullptr)
        imu_network_measurements[frame_count - 1]->translation_ +=
            imu_network_measurements[frame_count]->translation_;
